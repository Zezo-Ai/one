...
/**
 * Returns the total number of security groups in the virtual network.
 *
 * @param {VirtualNetwork} virtualNetwork - Virtual network
 * @returns {number} Total security groups
 */
export const getTotalSecurityGroups = ({ TEMPLATE = {} } = {}) => {
  const securityGroups = TEMPLATE?.SECURITY_GROUPS

  if (Array.isArray(securityGroups)) return securityGroups.length
  if (securityGroups && typeof securityGroups === 'object') {
    return [securityGroups?.ID ?? []].flat().filter(Boolean).length
  }

  return String(securityGroups ?? '')
    .split(',')
