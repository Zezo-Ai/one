...
import { isDate, timeToString } from '@UtilsModule'
...
/**
 * Returns schedule action DAYS as an array.
 *
 * @param {string|string[]|number} days - Scheduled action days
 * @returns {string[]} Days in array format
 */
export const getScheduleActionDays = (days = '') => {
  const values = Array.isArray(days) ? days : String(days ?? '').split(',')

  return values.map((day) => String(day).trim()).filter(Boolean)
}
...
export const getRepeatInformation = (action) => {
  ...
  const arrayDays = getScheduleActionDays(DAYS)
  const days = arrayDays.map((day) => daysOfWeek[day]).filter(Boolean)
  const daysValue = arrayDays.join(',')

  const repeat = {
    0: daysValue === ALL_DAYS ? `${T.Daily}` : `${T.Weekly} ${days.join(',')}`,
    1: `${T.Monthly} ${daysValue}`,
    2: `${T.Yearly} ${daysValue}`,
    3: [T.EachHours, daysValue],
  }[+REPEAT]
...
export const getRequiredArgsByAction = (action) => {
  ...
  return (
    {
      [BACKUP]: [DS_ID],
      [VM_ACTIONS.BACKUP_CREATE]: [DS_ID],
...
export const transformStringToArgsObject = ({ ACTION, ARGS = {} } = {}) => {
  ...
  return (
    {
      [BACKUP]: { [DS_ID]: arg1 },
      [VM_ACTIONS.BACKUP_CREATE]: { [DS_ID]: arg1 },
